...
async def _fetch_ohlcv(
    angelone: AngelOneProvider, instrument: ResolvedInstrument, timeframe: str
) -> pd.DataFrame:
    ...
    base_bars = await angelone.get_intraday_ohlc(instrument.exch_seg, instrument.token, base_interval, days)
    if base_bars.empty:
        return base_bars
...
async def run_futures_backtest(
    ...
    ohlcv = await _fetch_ohlcv(angelone, instrument, timeframe)
    if ohlcv.empty:
